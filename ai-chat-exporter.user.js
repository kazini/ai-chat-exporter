// ==UserScript==
// @name         ChatGPT / Claude / Copilot / Gemini / Google AI Mode == [Chat Exporter]
...
(function () {
  ...
  const GOOGLE_SEARCH = "google_search";
  const GOOGLE_SEARCH_HOSTNAMES = ["google.com", "www.google.com"];
  // Google AI Mode (SGE/AI Overviews) specific selectors.
  // Design principle: prefer data-xid and jsname attributes (stable, code-referenced)
  // over class names (generated by build tools, change frequently).
  //
  // Turn root: data-xid="aim-mars-turn-root" is a named stable ID — the safest anchor.
  const GOOGLE_SEARCH_TURN_ROOT_SELECTOR = '[data-xid="aim-mars-turn-root"]';
  // Inner wrapper of the turn root — jsname first, class as fallback.
  const GOOGLE_SEARCH_TURN_SELECTOR = "[jsname='RH7zg'], .tonYlb";
  // Individual turn container (one per user+AI exchange) — jsname first, class as fallback.
  const GOOGLE_SEARCH_TURN_ITEM_SELECTOR = "[jsname='CS7uPe'], .CKgc1d";
  // User query container — primary selector targets .sUKAcb (class), fallback to heading element.
  // .sUKAcb is more resilient to layout changes; heading selector is retained for robustness.
  const GOOGLE_SEARCH_USER_QUERY_SELECTOR = '.sUKAcb, [role="heading"][aria-level="2"]';
  // AI response content container — MUST have BOTH data-xid="VpUvz" AND jsname="KFl8ub"
  // simultaneously (not as alternatives) to ensure we target only the correct AI message div.
  // This avoids capturing wrapper divs (.Zkbeff, .pWvJNd) and the empty utility container [jsname="s7e4Od"].
  const GOOGLE_SEARCH_AI_RESPONSE_SELECTOR = 'div[data-xid="VpUvz"][jsname="KFl8ub"]';
  ...
  const CURRENT_PLATFORM = (() => {
    ...
    if (GOOGLE_SEARCH_HOSTNAMES.some((host) => HOSTNAME.includes(host))) {
  ...
  const ChatExporter = {
    ...
    /**
     * Extracts chat data from Google Search AI Mode (SGE/AI Overviews).
     * @param {Document} doc - The Document object.
     * @returns {object|null} The standardized chat data, or null.
     */
    extractGoogleSearchChatData(doc) {
      const turnRoots = doc.querySelectorAll(GOOGLE_SEARCH_TURN_ROOT_SELECTOR);
      if (turnRoots.length === 0) return null;

      const messages = [];
      let chatIndex = 1;
      let title = doc.title.replace(" - Google Search", "").trim() || DEFAULT_CHAT_TITLE;

      turnRoots.forEach((turnRoot) => {
        // .tonYlb / [jsname="RH7zg"] is the inner wrapper; inside it are one or more
        // turn item elements, each representing a single user-query + AI-response exchange.
        const turn = turnRoot.querySelector(GOOGLE_SEARCH_TURN_SELECTOR) || turnRoot;
        const turnItems = Array.from(turn.querySelectorAll(GOOGLE_SEARCH_TURN_ITEM_SELECTOR));
        // If no turn items found, fall back to treating the turn itself as one item
        const itemsToProcess = turnItems.length > 0 ? turnItems : [turn];

        itemsToProcess.forEach((item) => {
          // --- User query extraction ---
          // Try three approaches in order: [data-dq] for first turn, .sUKAcb container for all turns,
          // then fallback to nested heading element.
          let userText = "";
          
          // Approach 1: For the first turn, [data-dq] is the most reliable source
          if (chatIndex === 1) {
            const dqEl = doc.querySelector("[data-dq]");
            if (dqEl) userText = dqEl.getAttribute("data-dq");
          }
          
          // Approach 2: Target .sUKAcb directly and extract all text from span blocks within
          if (!userText) {
            const userQueryContainer = item.querySelector(GOOGLE_SEARCH_USER_QUERY_SELECTOR);
            if (userQueryContainer) {
              // Extract text from all span elements within the .sUKAcb container
              const spans = Array.from(userQueryContainer.querySelectorAll("span"));
              // Filter spans to get actual text content (skip buttons/UI elements)
              userText = spans
          ...
          // Approach 3: Broader fallback - any text content inside the container
          if (!userText) {
            const userQueryContainer = item.querySelector(GOOGLE_SEARCH_USER_QUERY_SELECTOR);
            if (userQueryContainer) userText = userQueryContainer.textContent.trim();
          }
          ...
          // IMPORTANT: The selector now targets BOTH data-xid="VpUvz" AND jsname="KFl8ub"
          // simultaneously to ensure we get only the AI message container, not wrapper divs.
          // Read innerText from the LIVE element before cloning — innerText requires CSS layout
          // and returns "" on detached nodes.
          const responseEl = item.querySelector(GOOGLE_SEARCH_AI_RESPONSE_SELECTOR);
          if (responseEl) {
            // Get the visible text content first
            const aiText = responseEl.innerText.trim();
            
            // Clone the element and aggressively clean unwanted elements
            const clone = responseEl.cloneNode(true);
            
            // Remove all non-text elements:
            // - script, style, noscript: code/styling
            // - img, svg: images and base64 data
            // - button, [role="button"]: citation buttons and interactive elements
            // - [style*="display:none"]: hidden elements
            // - .uJ19be, .notranslate, .vKEkVd, .rBl3me, .IWyTpf: citation/UI containers
            const selectorsToRemove = [
              "script, style, noscript",
              "img, svg",
              "button, [role='button']",
              "[style*='display:none']",
              ".uJ19be, .notranslate, .vKEkVd, .rBl3me, .IWyTpf, .pjvauc",
              "[data-amic], [data-icl-uuid]", // Citation elements
              ".Fwa2Od, .wiMplc, .ofC0Ud", // Citation UI
            ];
  ...
  const UIManager = {
    ...
    getTargetContentWidth() {
      ...
      } else if (CURRENT_PLATFORM === GOOGLE_SEARCH) {
        // Try to find the search results container
        targetElement = document.querySelector(".srp") || document.getElementById("center_col");
      }
    ...
    generateOutlineContent() {
      ...
      switch (CURRENT_PLATFORM) {
        ...
        case GEMINI:
          ...
          break;
        case GOOGLE_SEARCH:
          freshChatData = ChatExporter.extractGoogleSearchChatData(document);
          break;
        default:
    ...
    initObserver() {
      ...
      switch (CURRENT_PLATFORM) {
        ...
        case GEMINI:
          ...
          break;
        case GOOGLE_SEARCH:
          targetNode = document.querySelector("#main") || document.querySelector("#cnt") || document.body;
          break;
        default:
